...
function updateStats(): void {
  ...
  if (statLinesElement)
    statLinesElement.textContent = String(parsedLines.length);
  if (statDurationElement)
    statDurationElement.textContent = `${Math.floor(duration / 60)}m`;
  if (statAvgGapElement)
    statAvgGapElement.textContent = `${avgGap.toFixed(1)}s`;
  if (statSpeakersElement)
    statSpeakersElement.textContent = String(speakers.size);
...
(globalThis as typeof globalThis & GlobalFunctions).parseTranscript =
  parseTranscript;
