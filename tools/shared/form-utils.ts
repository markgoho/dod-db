...
export function getTagFormData({
  ...
  const canonical = (document.querySelector(canonicalSelector) as HTMLInputElement)?.value.trim() || '';
  const variations = (document.querySelector(variationsSelector) as HTMLInputElement)?.value.trim() || '';
  const category = (document.querySelector(categorySelector) as HTMLSelectElement)?.value || '';
  const llmVerify = (document.querySelector(llmVerifySelector) as HTMLInputElement)?.checked || false;
  const caseSensitive = (document.querySelector(caseSensitiveSelector) as HTMLInputElement)?.checked || false;
  const description = (document.querySelector(descriptionSelector) as HTMLTextAreaElement)?.value.trim() || '';
...
export function clearTagForm({
  ...
  const descriptionGroup = document.querySelector(descriptionGroupSelector) as HTMLElement;
...
export function validateTagForm(data: TagFormData): { valid: boolean; error?: string } {
  ...
  if (data.llmVerify && !data.description) {
    return { valid: false, error: 'Description is required when using LLM verification' };
  }
...
export function toggleDescriptionField({
  ...
  const llmVerifyCheckbox = document.querySelector(checkboxSelector) as HTMLInputElement;
  const descriptionGroup = document.querySelector(descriptionGroupSelector) as HTMLElement;
  const descriptionField = document.querySelector(descriptionFieldSelector) as HTMLTextAreaElement;
