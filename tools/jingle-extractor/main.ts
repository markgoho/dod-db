...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const audioFileInput = document.querySelector('#audioFile') as HTMLInputElement;
  ...
  (globalThis as unknown as { zoomToRange: () => void }).zoomToRange = zoomToRange;
...
function updateSelection(): void {
  ...
  if (startMarker !== null && endMarker !== null) {
    ...
    // Show ffmpeg command
    const commandDiv = document.querySelector('#ffmpegCommand') as HTMLDivElement;
    const commandText = document.querySelector('#commandText') as HTMLPreElement;
...
function parseTimeString(timeString: string): number {
  ...
  if (parts.length === 2) {
    return Number.parseInt(parts[0] ?? '0', 10) * 60 + Number.parseFloat(parts[1] ?? '0');
  }
