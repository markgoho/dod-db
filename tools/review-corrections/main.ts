...
async function jumpToTimestamp(timestamp: string, episodeId: string): Promise<void> {
  ...
  if (currentEpisodeId !== episodeId) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Audio file not found for episode ${episodeId}. Please upload manually.`);
        return;
      }
    } catch (error) {
...
async function loadAudioForCandidate(candidate: Candidate): Promise<void> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      audioStatusElement.textContent = '⚠️ Audio not found. Use manual upload below.';
    }
  } catch (error) {
    console.error('Failed to load audio:', error);
    if (audioStatusElement) {
      audioStatusElement.textContent = '⚠️ Failed to load audio. Use manual upload below.';
    }
  }
}
...
async function loadCandidates(): Promise<void> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/review-corrections/candidates`);
    ...
    if (pendingCountElement) pendingCountElement.textContent = String(pending.length);
    if (reviewedCountElement) reviewedCountElement.textContent = String(reviewed.length);
    if (totalCountElement) totalCountElement.textContent = String(allCandidates.length);
    ...
    container.innerHTML = pending
      .map((candidate) => {
        const confidenceClass =
          candidate.confidence >= 70 ? 'high' : (candidate.confidence >= 50 ? 'medium' : 'low');
        ...
        const examplesHtml = candidate.examples
          .map((ex, index) => {
            ...
            const timestamp = candidate.timestamps && candidate.timestamps[index];
            const episodeId = candidate.episodeIds && candidate.episodeIds[index];
...
async function approve(key: string): Promise<void> {
  ...
  const original = originalElement
    ? originalElement.textContent?.replaceAll(/^"|"$/g, '') || key.split('→')[0] || ''
    : key.split('→')[0] || '';
  const corrected = correctedElement
    ? correctedElement.textContent?.replaceAll(/^"|"$/g, '') || key.split('→')[1] || ''
    : key.split('→')[1] || '';

  if (!confirm(`Add [["${original}"], "${corrected}"],\nto corrections.ts?`)) return;
...
async function reject(key: string): Promise<void> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/review-corrections/reject/${encodeURIComponent(key)}`, {
      method: 'POST',
    });
...
function removeCandidate(key: string, message: string): void {
  const candidateElement = document.querySelector(`#candidate-${safeEncode(key)}`);
...
document.addEventListener('keydown', (e) => {
  ...
  // Review shortcuts
  const candidates = [...document.querySelectorAll('.candidate:not(.removing)')];
...
(globalThis as typeof globalThis & GlobalFunctions).jumpToTimestamp = jumpToTimestamp;
...
(globalThis as typeof globalThis & GlobalFunctions).showEpisodeInfo = showEpisodeInfo;
