...
function updateStats(): void {
  const totalTags = allEpisodes.reduce((sum, ep) => sum + (ep.tags?.length || 0), 0);
  const totalSegments = allEpisodes.reduce((sum, ep) => sum + (ep.segments?.length || 0), 0);
  ...
  if (totalEpisodesElement) totalEpisodesElement.textContent = String(allEpisodes.length);
  if (totalTagsElement) totalTagsElement.textContent = String(totalTags);
  if (totalSegmentsElement) totalSegmentsElement.textContent = String(totalSegments);
}
