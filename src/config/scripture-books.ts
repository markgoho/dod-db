...
/**
 * Get a book definition by canonical name.
 */
export function getBookByCanonical(name: string): BookDefinition | undefined {
  return scriptureBooks.find(
    book => book.canonical.toLowerCase() === name.toLowerCase(),
  );
}

/**
 * Get a book definition by any name (canonical, abbreviation, or variant).
 * Returns the matching book definition or undefined if not found.
 */
export function getBookByAnyName(name: string): BookDefinition | undefined {
  const normalizedName = name.toLowerCase();

  return scriptureBooks.find(book => {
    // Check canonical name
    if (book.canonical.toLowerCase() === normalizedName) {
      return true;
    }

    // Check abbreviations
    if (book.abbreviations.some(abbr => abbr.toLowerCase() === normalizedName)) {
      return true;
    }

    // Check variants
    if (book.variants.some(variant => variant.toLowerCase() === normalizedName)) {
      return true;
    }
...
/**
 * Get all books that require LLM verification.
 */
export function getAmbiguousBooks(): BookDefinition[] {
  return scriptureBooks.filter(book => "llmVerify" in book && book.llmVerify);
}
