...
import { parseTimestampToSeconds, MAX_DURATION_SECONDS } from "../utils/collapse-transcript.js";
...
function collapseLines(content: string): string {
	...
	for (const line of lines) {
		...
		if (!match) {
			...
			console.warn(`  Warning: Line does not match expected format: ${trimmed.slice(0, 60)}...`);
			continue;
		}
		...
		if (timestamp === undefined || speaker === undefined || text === undefined) {
			console.warn(`  Warning: Unexpected match structure: ${trimmed.slice(0, 60)}...`);
			continue;
		}
...
async function processFile(filePath: string, dryRun: boolean): Promise<{ before: number; after: number }> {
...
`);
	...
	if (values.all) {
		...
		files = positionals.map((f) => (path.isAbsolute(f) ? f : path.join(process.cwd(), f)));
	} else {
	...
	for (const file of files) {
		...
		console.log(`  ${before} lines → ${after} lines (${reduction} lines collapsed, ${percent}% reduction)\n`);
	}

	console.log("---");
	console.log(`Total: ${totalBefore} lines → ${totalAfter} lines`);
	console.log(`Collapsed: ${totalBefore - totalAfter} lines (${(((totalBefore - totalAfter) / totalBefore) * 100).toFixed(1)}% reduction)`);
