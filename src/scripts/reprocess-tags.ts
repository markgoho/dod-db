...
async function main() {
  ...
  const categoriesArgument = process.argv.find(argument =>
    argument.startsWith("--categories="),
  );
  ...
  if (skipLlm) {
    console.log(
      "⚡ Skip LLM mode: Deterministic matching only, no LLM verification (zero API costs)\n",
    );
  }
  if (categories) {
    console.log(
      `🎯 Category filter: LLM will only discover tags in [${categories.join(", ")}]\n`,
    );
  }
  if (verbose) {
  ...
  // Run reprocessing
  const result = await reprocessEpisodes({
    force,
    skipLlm,
    categories,
    verbose,
