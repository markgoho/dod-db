...
import { calculateCost, aggregateCosts, formatCost } from '../lib/cost-calculator.js';
import {
...
async function runParallelSingle(
  ...
  console.log(`    Wall clock: ${(parallelResult.wallClockMs / 1000).toFixed(1)}s`);
  console.log(`    Total processing: ${(parallelResult.totalProcessingMs / 1000).toFixed(1)}s`);
...
async function runAllConcurrencyLevels(): Promise<void> {
  ...
  for (const concurrency of CONCURRENCY_LEVELS) {
    ...
    await writeResults('parallel', results, {
      ...summary,
      recommendations: [`Concurrency: ${concurrency}`, ...summary.recommendations],
    });
  }
...
async function main(): Promise<void> {
  ...
  if (!values.concurrency) {
    console.log('Usage:');
    console.log('  bun run experiments/runners/parallel-experiment.ts --concurrency=4');
    console.log('  bun run experiments/runners/parallel-experiment.ts --concurrency=full');
    console.log('  bun run experiments/runners/parallel-experiment.ts --all-concurrency');
    return;
  }
  ...
  if (values.transcript === undefined) {
    ...
    results = [
      await runParallelSingle(sample.raw, sample.truth, sample.name, concurrency),
    ];
  }
