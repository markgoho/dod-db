---
title: "Second Coming"
definition: "The Second Coming is the expected return of Jesus in Christian eschatology, often tied to judgment, resurrection, tribulation, and the kingdom of God. It functions less as a single clear biblical event than as a composite doctrine assembled from multiple texts and interpreted in sharply different ways."
featuredItems:
  - type: episode
    number: 32
    label: "The End is Near?"
  - type: episode
    number: 107
    label: "Immigrant Apocalypse"
quotes:
  - text: "The idea here is that this second coming, Jesus's kingdom arriving on earth, will happen within the first generation of followers of Jesus."
    speaker: "Dan McClellan"
    episode: 32
    timestamp: "00:15:26.120"
  - text: "We've talked in the past about how the Olivet Discourse makes it sound like the second coming is going to happen immediately after the events that are being described. And the events that are being described sound an awful lot like what happened around the year 70 CE when Rome had been laying siege to Judea for a while and, and destroyed Jerusalem, destroyed the temple in Jerusalem, ran off the Jewish rebels."
    speaker: "Dan McClellan"
    episode: 119
    timestamp: "00:05:42.140"
  - text: "Usually the Second Coming is what ushers in that millennium where we have a thousand years of peace and Jesus reigning on the earth."
    speaker: "Dan McClellan"
    episode: 71
    timestamp: "00:06:33.090"
  - text: "And, and there's a legend within Mormonism that everybody's going to gather there for the apocalypse, the second coming and all that. So it seems like it was kind of in the air."
    speaker: "Dan McClellan"
    episode: 107
    timestamp: "00:08:34.180"
showTopEpisodes: true
topEpisodesLimit: 6
showAllEpisodes: true
aliases:
  - "/tags/second-coming/"
---

Second Coming comes up on the show as a way into larger debates about prophecy, apocalyptic expectation, and how readers stitch together Daniel, the Gospels, Paul, and Revelation into one future timetable. The recurring argument is that the doctrine people often treat as straightforward is actually produced by negotiation among texts that do not line up neatly, especially once failed first-century expectations are taken seriously.

The topic also matters because it shapes present-day behavior. Episodes tied to this topic connect Second Coming belief to anxiety, speculative end-times culture, and political uses of scripture, including the tendency to interpret wars, pandemics, or nationalism as signs that history is reaching its climax.
